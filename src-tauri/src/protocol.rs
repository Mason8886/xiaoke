...
//! on stderr. XiaoKe responds via stdin with `control_response` messages.
//!
//! Note: Permission responses (XiaoKe → CLI) are built as raw `serde_json::json!`
...
// ─── XiaoKe → CLI: SDK control requests (stdin) ─────────────────────────────
...
/// SDK control request subtypes that XiaoKe can send to CLI.
#[derive(Debug, Serialize)]
#[serde(tag = "subtype")]
