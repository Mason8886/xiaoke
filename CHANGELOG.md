...
- **CLI 更新进度条** — 更新 CLI 时显示阶段说明和进度条，不再只有转圈动画
- **npm 镜像版本校验** — npmmirror 同步延迟时自动回退 npm 官方源重试，确保装到最新版
- **版本检查修复** — 版本检查走 herear.cn 镜像 + 语义化比较，修复同版本误报「有新版本」
- **设置页 CLI 红点** — CLI 管理标签显示红点提示，一眼知道是什么要更新

## [0.9.8] - 2026-04-04

### Fixed

- **CLI 更新走自有镜像** — 版本检查优先走 herear.cn 镜像，不再依赖可能被墙的 GCS
...
- **文件夹图标区分度** — 文件夹图标使用主题色显示（亮色模式 70% 透明度，暗色模式全量），与文件图标形成自然区分。
