  ...
  load: async () => {
    try {
      ...
      if (data.providers.length === 0) {
        ...
        if (migrated) {
          const normalized = normalizeProvider(migrated);
          data.providers = [normalized];
          data.activeProviderId = normalized.id;
          // Save migrated data
          await bridge.saveProviders(data);
          console.log('[providerStore] Migrated old API settings to provider:', normalized.name);
        }
      }

      set({
        providers: (data.providers as ApiProvider[]).map(normalizeProvider),
...
function migrateFromSettingsStore(): ApiProvider | null {
  try {
    ...
    const raw = localStorage.getItem('xiaoke-settings');
    if (!raw) return null;
